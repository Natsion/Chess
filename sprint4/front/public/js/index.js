...
function showError(message) {
    ...
    // Set a new timeout to hide the error
    errorTimeout = setTimeout(() => {
        errorMessage.classList.remove('show');
    }, 3000); // Extend timer to 5 seconds
}
...
function handleAction(buttonId, action, cost) {
    ...
    if ((playerColor === 'white' && game.turn() !== 'w') || (playerColor === 'black' && game.turn() !== 'b')) {
        showError("You can only perform this action on your turn!");
    ...
    if (coinBalance >= cost) {
        ...
        if (action === "seeOpponentCoins") {
            socket.emit('replicateMessage', "used [Lemme See]", playerColor);
            socket.emit("seeOpponentCoins", playerColor); // Request opponent’s coin balance
        }

        if (action === "activateMultiplier") {
            socket.emit('replicateMessage', "used [Multiplier]", playerColor);
            multiplierActive = true; // Activate multiplier for next capture
            socket.emit("activateMultiplier", playerColor); // Notify opponent
            makeMessage(`Multiplier Activated: Your next capture is worth 2x points.`);
        }

        // Perform the selected action
        if (action === "flipBoard") {
            socket.emit('replicateMessage', "used [Flip Board]", playerColor);
            socket.emit("flipBoard", playerColor);
        } else if (action === "reduceOpponentCoins") {
            socket.emit('replicateMessage', "used [Suckin' It]", playerColor);
            ...
            if (pieces.length > 0) {
                socket.emit('replicateMessage', "used [Russian Roulette]", playerColor);
            ...
            socket.emit('replicateMessage', "used [Inflation]", playerColor);
            socket.emit("increaseOpponentCost", playerColor);
        } else if (action === "gambleCoins") {
            socket.emit('replicateMessage', "used [Double or Nothing]", playerColor);
            ...
            if (opponentPieces.length > 0) {
                ...
                socket.emit('replicateMessage', "used [Rizzler]", playerColor);
            } else {
            ...
            if (validMoves.length > 0) {
                socket.emit('replicateMessage', "used [Pray to God]", playerColor);
                ...
                errorMessage("No valid moves available!");
            }
        }
    } else {
        errorMessage("Not enough coins for this action!");
    }
}
...
function makeMessage(msg, error) {
    const message = document.createElement('div');
    message.textContent = "S: " + msg;
    message.classList.add('system-message'); // Add right-aligned class
...
function updateButtonTextWithCost() {
    ...
    buttons.forEach((button) => {
        const baseCost = parseInt(button.getAttribute("data-cost"), 10);
        const adjustedCost = Math.ceil(baseCost * costMultiplier); // Apply updated multiplier
        const costSpan = button.querySelector(".ability-cost");

        if (costSpan) {
            costSpan.textContent = `${adjustedCost} Coins`; // Update displayed cost
        }
    });
}
...
socket.on("displayOpponentCoins", (opponentCoinBalance) => {
    makeMessage(`Your opponent currently has ${opponentCoinBalance} coins.`);
});
...
function activateMysteryBox() {
    ...
    if (coinBalance >= 1) { // Check if player has enough coins for the Mystery Box
        ...
        makeMessage(`Mystery Box -> You got the "${randomAbility}" ability.`);
    }
}
